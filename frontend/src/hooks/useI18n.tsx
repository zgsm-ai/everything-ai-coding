...
const translations: Record<string, Record<Lang, string>> = {
  ...
  'detail.install.skillExtract': { en: 'Use the direct source path below for this skill.', zh: '该技能请直接使用下面的源码直达路径。' },
  'detail.install.sourcePathLabel': { en: 'Direct source path', zh: '直达路径' },
  'detail.install.skillExtractHint': { en: 'Open the linked path and install only that skill directory into your local skills folder.', zh: '打开这个直达路径，只安装对应的 skill 目录到本地技能目录。' },
  ...
  'about.pipeline.step2.label': { en: 'Merge', zh: '合并' },
  'about.pipeline.step2.desc': { en: 'Deduplicate entries across sources, merge metadata', zh: '跨源去重，合并元数据' },
  'about.pipeline.step3.label': { en: 'Evaluate', zh: '评估' },
  'about.pipeline.step3.desc': { en: 'LLM scores 6 dimensions + enrichment (tags, summary, tech_stack)', zh: 'LLM 评 6 个维度 + 富化字段（tags、summary、tech_stack）' },
  'about.pipeline.step4.label': { en: 'Govern', zh: '决策' },
  'about.pipeline.step4.desc': { en: 'LLM × 0.85 + health × 0.15 → accept / review / reject', zh: 'LLM × 0.85 + 健康度 × 0.15 → 接受 / 复审 / 拒绝' },
  'about.scoring.title': { en: 'Scoring Weights', zh: '评分权重' },
  'about.scoring.desc': { en: 'LLM rates up to 6 dimensions (1–5), weighted by type. Combined 85/15 with health signals into', zh: 'LLM 对最多 6 个维度评 1–5 分，按类型加权。LLM 占 85% + 健康度 15%，混合为' },
  'about.thresholds.title': { en: 'Decision Thresholds', zh: '决策阈值' },
  'about.health.title': { en: 'Health Score', zh: '健康度评分' },
  'about.health.desc': { en: '15% of final_score — measures repository vitality via heuristic signals.', zh: '占 final_score 的 15%——通过启发式信号衡量仓库活跃度。' },
  ...
  'about.signal.sourceTrust': { en: 'Source Trust', zh: '来源可信度' },
  'about.signal.sourceTrust.desc': { en: 'Upstream reputation (Anthropic = 5, mcp.so = 2)', zh: '上游声誉（Anthropic = 5, mcp.so = 2）' },
