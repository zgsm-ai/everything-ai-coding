...
- **已入库 monorepo 增量重扫**（`06-23-monorepo-incremental-rescan` change 引入）：补"仓一旦入库 → 进 `known_repos` → Stage A 永久预过滤跳过 → 上游后续新增的 skill 永远收不进来"的盲区（实测 mattpocock/skills 入库时 32 个 SKILL.md、现仓 34，`qa`/`review` 是入库后加的）。
  - **范围**：从 catalog 取 `source ∈ {github-trending} ∪ 促升 source_slug 集` 且 `type==skill` 的 entry，从 `source_url` 反解唯一 `owner/repo`（小写归一），即 `sync_github_trending.build_rescan_scope`。**排除 Tier-2**（`skill_registry.discover_skills` 扫 `skill_repos.json` 白名单每轮本就重扫，其 source 不在此集，天然排除）；**plugin 不做**（bundle 重检复杂，后续）。
  - **变更检测 = pushed_at 门**：状态表 `.github_trending_cache/scanned_repos.json`（`{owner/repo: last_scanned_pushed_at}`）。每轮对范围仓拉一次 `repos/<owner>/<repo>` 元数据比 `pushed_at`——**变新 → 需重扫**；未变 → 跳过（省成本，无新 skill 时至多浪费一次拉树、不调 LLM）；**cache 无此仓（首次见）→ 也需重扫**（本任务动机正是抓回已入库 monorepo 在基线时刻**已新增**的 skill，如 mattpocock 的 `qa`/`review`；首见若只记基线则永远判不出"新"、永不抓回。首见与"变新"一视同仁，靠 `MAX_RESCAN` 限量分轮收敛，~ceil(范围/N) 轮内 backlog 排空）。
  - **Stage A 豁免注入**：`discover_candidates` 在 known_repos 预过滤 + seed 注入**之后**，把需重扫仓（本就在 known_repos、正常会被挡）作为候选注入（字段与 search item 同构，**带真实 stars** —— hard_filter 有 `stars<=50` 闸，占位 0 会误杀本仓所有新 skill），按 `full_name` 去重。`MAX_RESCAN`（env `TRENDING_MAX_RESCAN`，默认 30）限每轮重拉树仓数，超出按 pushed_at 最新优先、其余推迟下轮。
  - **triage 只加新 + cache 时序**：重扫候选带新 pushed_at（或首见无 verify_cache 记录）→ triage 的 `verify_cache`（旧 pushed_at）不命中 → 重新深拉 → `merge_preserve`（按 id + 归一 url）**只加库里没有的新 skill**（存量不动、不重评）；促升仓的专属 slug 在 triage promote 路由照常套用。**只有实际重扫成功产出 skill entry 才**把该仓写进 `scanned_repos.json`（`triage` 流程 `_do_flush` 落盘）；失败 / 0 产出 / plugin 仓 / **被 `MAX_RESCAN` 推迟（未实际重扫）的仓 → 不更新**（下轮继续重扫，backlog 不丢）。`discover_candidates` 检测阶段**不写 baseline**（避免推迟仓被误记基线后下轮不再重扫）。健康度经 triage WARN 的 `written skill` 计数可见（=本轮实际新增 skill 数，含重扫长出的）。
...
- **CI**：Stage A 在 sync-data job 跑（纯搜索，10min timeout）；triage 紧随其后、**merge 之前**跑（40min timeout + LLM secrets，与 enrich 同款），保证新 entry 进本周期 `catalog/index.json`。`.github_trending_cache/`（整目录 weekly cache block，含 `candidates.json` / `verify_cache.json` / `scanned_repos.json` 增量重扫状态）+ `.eval_cache/`（triage authenticity）各自独立 cache block。
- **覆盖**：skill + plugin（MVP）。star velocity / trending 时间序列信号留待后续。

### 前端数据层（slim search-index + per-entry 分片）

`scripts/build_frontend_data.py` 从 `catalog/index.json` 生成 GitHub Pages 前端消费的全部 `frontend/public/api/*` 文件（`stats.json` / `featured.json` / 各 per-type 列表 / `sources.json` / **`search-index.json`** / **`entries/<bucket>.json`**）。由 `publish-site.yml` 在部署前跑。`search-index-perf-refactor` change 重构了搜索数据层（21MB 巨索引 + 客户端 O(n) 全表扫的两大病灶）：

- **slim `search-index.json`（权威生成者 = `build_frontend_data.py`，不再 `shutil.copy2` merge_index 那份）**：每条只留**最小卡片字段** `id / name / type / source / stars / final_score / freshness_label / snippet（描述截断 ~160 字）`，外加索引用 **`search_text`** 与分片号 **`shard`**（见下）。**移出**重字段（全 description / description_zh / install / bundled_in / tech_stack / install_method / source_url）→ 落到 per-entry 分片。原始 ~11MB（23590 条，real description 不可压），**gzip 后 ~2.1MB**（GitHub Pages 服务端 gzip/brotli，决定首屏的是 wire 体积，对比旧版 gzip 3.46MB / 原始 21MB）。`build_search_entry()` 产 slim entry；`build_search_index()` 产数组。
  - 注：`catalog/search-index.json`（`merge_index.py:1472` + `aggregate_enrichment.py` 写）与前端那份**已解耦**——它只喂 `generate_pages.py` 的旧 `/api/v1/` 静态树（前端实际 fetch 的是 `./api/search-index.json` = build_frontend_data 产物，非 v1）。
- **`search_text` 收录来源/作者（解决"按来源搜不到"）**：`build_search_text()` = tags + **source（如 `mattpocock/skills`）+ owner-repo + owner（作者，从 `source_url` 反解，如 `mattpocock`）** + LLM `search_terms`。**刻意不重复 name/description**（那两个是各自独立的 MiniSearch 字段——PR2 把 `name` / `snippet` / `search_text` 都 index，prose 只存一份，避免旧 21MB 索引的字段双写）。效果：搜 `mattpocock`/`matt` 命中该来源全部 33 条（旧版仅名字带 matt 的 2 条）。
- **`source` 字段填充**：旧 search-index 22927 条 `source` 全空；新版每条从 catalog entry 的 `source` 透传，非空（slim entry `source` 字段）。
- **per-entry 全字段分片（`api/entries/<bucket>.json`）**：供 Detail 页按需拉单条，替代旧"一次性加载 5 个 per-type 大文件"。**不生成 22927 个独立文件**（仓库/部署会炸）——按 `shard_bucket(id)`（md5(id) 前 8 hex % `ENTRY_SHARD_BUCKETS=256`）分到固定 256 桶，每桶 `{id: full_entry}` map（~92 条/桶、~210KB），**文件数恒定 256 不随 catalog 增长**。Detail 端 fetch 一个文件即 O(1) 取回全字段（`build_entry_shards()` / `write_entry_shards()`，复用 `slim_item()` 装齐 Detail 渲染所需的 description/install/health/evaluation/bundle/security/mcp_* 等全字段；每轮先清空 `entries/` 旧分片防残留）。
- **分片号 `shard` 内联进 slim entry（PR2 加，浏览器无 MD5）**：浏览器 `SubtleCrypto` 不支持 MD5，没法在 JS 端复算 `shard_bucket`。所以 `build_search_entry()` 把每条 slim entry 的分片号写成一个整数 **`shard`** 字段（与 per-entry 分片用同一 `shard_bucket()` 算，单测 `test_build_frontend_data_search.py` 锁定二者一致）。前端 Detail 直接读 `entry.shard` 拉 `api/entries/<shard>.json`，**JS 里零哈希**。
- **PR 拆分**：PR1（纯数据层）产 slim search-index + 分片（含 `shard` 字段），保证是 MiniSearch 能直接 `addAll` 的形状。**PR2（已实现）= 前端搜索 + Detail 按需**：`useSearch.ts` 换 **MiniSearch**——`fetch('./api/search-index.json')` → `buildSearchIndex()`（`new MiniSearch({fields:['name','snippet','search_text'], searchOptions:{boost:{name:3,snippet:1,search_text:0.8}, prefix:true, fuzzy:0.2, combineWith:'AND'}})` + `addAll`，模块级缓存只建一次），查询走 `ms.search(q)`（干掉旧 O(n) 逐 token 全表扫 + 自研 rerank）；slim hit 经 `slimToCard()` 适配成 ResourceCard 能渲染的 `CatalogItem`（`snippet`→`description`、`freshness_label`→`health.freshness_label`，category/tags 等重字段缺省）。`Detail.tsx` 改 **按需拉单条**：读 search-index 拿 slim entry → `entry.shard` → fetch 那一个 `entries/<shard>.json` → `data[id]` 取全字段；分片缺失/取不到回退 slim entry（`slimToFallbackItem()` 补齐缺省字段防渲染崩）。`Browse.tsx` 搜索态禁用 category 下拉（slim 结果无 category），保留 MiniSearch 相关性排序。搜索逻辑由 `useSearch.test.ts`（vitest）覆盖（mattpocock 全源召回 + name boost 排序）。**PR3（已实现）= 结果列表虚拟滚动**：`Browse.tsx` 把结果网格换成 `VirtualGrid`（`@tanstack/react-virtual` 的 `useWindowVirtualizer`，沿用 document window 滚动不引内层滚动条），按 `gridColumns.ts:columnsForWidth()`（镜像 Tailwind `grid-cols-1 sm:2 lg:3` 的 sm=640/lg=1024 断点）把 N 张卡打包成行、只挂载视口内行 + overscan，DOM 节点数 = `已挂载行 × 列数`，与结果总量解耦（上万条不淹 DOM）；行高 `measureElement` 动态量避免裁切。纯函数 `columnsForWidth` / `rowCount` / `rowItems` 拆到 `gridColumns.ts` 由 `VirtualGrid.test.ts`（vitest）锁定行打包契约 + DOM 受控不变量。
